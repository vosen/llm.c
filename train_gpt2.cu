...
void gpt2_forward(GPT2 *model, const int* inputs, size_t B, size_t T) {
    ...
    for (int l = 0; l < L; l++) {
        ...
        attention_forward_cudnn(scratch, l_atty, (float*)l_att, l_qkvr, B, T, NH, C, main_stream);
        #else
...
void gpt2_backward_and_reduce(GPT2 *model, int* inputs, const int* targets, int grad_accum_steps, int micro_step) {
    ...
    for (int l = L-1; l >= 0; l--) {
        ...
        #ifdef ENABLE_CUDNN
        float* l_att = (float*)acts.att + l * B * NH * T; // cuDNN needs a smaller FP32 tensor
        attention_backward_cudnn(scratchX, dl_bt4c, dl_btc, l_qkvr, l_atty, (float*)l_att, B, T, NH, C, main_stream);
        #else
        floatX* l_att = acts.att + l * B * NH * T * T;
